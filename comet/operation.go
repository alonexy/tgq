...
import (
	...
	model "github.com/cenwj/quote/api/comet/grpc"
	// logic "github.com/cenwj/quote/api/logic/grpc"
	"github.com/cenwj/quote/pkg/strings"
...
func (s *Server) Connect(c context.Context, p *model.Proto, cookie string) (mid int64, key, rid string, accepts []int32, err error) {
	...
	if err = json.Unmarshal(p.Body, &reply); err != nil {
		log.Errorf("json.Unmarshal(%s) error(%v)", p.Body, err)
		return
	}

	if reply.Key == "" {
		reply.Key = uuid.New().String()
	}
